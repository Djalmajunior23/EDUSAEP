...
import { DiagnosticResult, generateSuggestions, generateRecoveryPlan } from '../../services/geminiService';
...
export function AlunoView({ result, onUpdateResult, diagnosticId, userProfile, history, classAverages, selectedModel }: AlunoViewProps) {
  ...
  const exportToPDF = async () => {
    ...
    setIsExporting(true);
    try {
      await pdfExportService.exportElementToPDF(reportRef.current, 'relatorio_diagnostico.pdf');
  ...
  const handleSaveEdit = async (competenciaName: string, type: string, value?: string | number, questionId?: string | number, nota?: string | number) => {
    if (!result) return;
    ...
    if (type === 'studentMessage') {
      ...
      } else if (type === 'questionFeedback' || type === 'questionNota' || type === 'fullQuestionFeedback' || type === 'questionPrivateNote') {
    ...
    if (diagnosticId) {
      try {
        ...
        handleFirestoreError(err, OperationType.UPDATE, `diagnostics/${diagnosticId}`);
      }
    }
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          >
            {generatingPlan ? <Loader2 className="animate-spin" size={14} /> : <Brain size={14} />} Gerar Plano
          </button>
          <button
            onClick={exportToPDF}
      ...
      <div ref={reportRef} className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 pb-6 border-b border-gray-100">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 font-medium tracking-wide uppercase">Relatório Individual</p>
            </div>
          </div>
          <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-200 shadow-sm flex flex-col md:w-1/2 relative group">
            <h3 className="text-sm font-bold text-emerald-900 mb-2">Mensagem para o Aluno</h3>
            {isProfessor && editingComp !== 'studentMessage' && (
              <button 
                onClick={() => { setEditingComp('studentMessage'); setEditValue(result.mensagem_para_o_aluno); }}
            ...
            {editingComp === 'studentMessage' ? (
              <div className="space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={() => setEditingComp(null)} className="text-xs font-bold">Cancelar</button>
                  <button onClick={() => handleSaveEdit('', 'studentMessage')} className="px-3 py-1 bg-emerald-600 text-white rounded font-bold">Salvar</button>
                </div>
              </div>
            ) : <p className="text-lg leading-relaxed text-emerald-800 italic">"{result.mensagem_para_o_aluno}"</p>}
          </div>
        </div>
        ...
        {result.diagnostico_por_competencia.some(c => c.questoes?.some(q => !q.acertou && q.analise_erro)) && (
          <div className="mb-8 p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
            <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2"><Brain size={20} className="text-amber-600" /> Perfil de Dificuldades Cognitivas</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {['Interpretação', 'Conceito', 'Atenção', 'Lógica'].map(cat => {
                const count = result.diagnostico_por_competencia.reduce((acc, c) => acc + (c.questoes?.filter(q => !q.acertou && q.analise_erro?.categoria === cat).length || 0), 0);
                if (count === 0) return null;
                return (
                  <div key={cat} className={cn("p-4 rounded-xl border text-center", cat === 'Interpretação' ? 'bg-blue-50 border-blue-100 text-blue-900' : cat === 'Conceito' ? 'bg-emerald-50 border-emerald-100 text-emerald-900' : cat === 'Atenção' ? 'bg-amber-50 border-amber-100 text-amber-900' : 'bg-red-50 border-red-100 text-red-900')}>
        ...
        <div className="space-y-8">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs" />
              <select value={filter} onChange={(e) => setFilter(e.target.value as any)} className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs cursor-pointer">
          ...
          <div className="grid grid-cols-1 gap-6">
            {filteredCompetencias.map((comp, idx) => (
              <motion.div key={idx} className={cn("p-6 bg-white border rounded-2xl shadow-sm", comp.nivel === 'Crítico' ? "border-red-400 bg-red-50/10" : "border-gray-100")}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={cn("w-3 h-3 rounded-full", comp.nivel === 'Forte' ? 'bg-emerald-500' : comp.nivel === 'Atenção' ? 'bg-amber-500' : 'bg-red-500')} />
                    <h4 className="text-lg font-bold">{comp.competencia}</h4>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-black">{Math.round(comp.acuracia_ponderada * 100)}%</p>
                    <p className={cn("text-[10px] font-bold uppercase", comp.nivel === 'Forte' ? 'text-emerald-600' : comp.nivel === 'Atenção' ? 'text-amber-600' : 'text-red-600')}>{comp.nivel}</p>
                  </div>
                </div>
                ...
                <div className="mb-6 space-y-1">
                  <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase">
                    <span>Média da Turma: {Math.round(classAverages[comp.competencia] || 0)}%</span>
                    <span>Status: {Math.round(comp.acuracia_ponderada * 100) >= (classAverages[comp.competencia] || 0) ? 'Acima' : 'Abaixo'}</span>
                  </div>
                  <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                    <div className={cn("h-full", comp.nivel === 'Forte' ? 'bg-emerald-500' : comp.nivel === 'Atenção' ? 'bg-amber-500' : 'bg-red-500')} style={{ width: `${comp.acuracia_ponderada * 100}%` }} />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <p className="text-[10px] font-bold text-gray-400 uppercase">A Reforçar</p>
                    <div className="flex flex-wrap gap-2">
                      {comp.conhecimentos_fracos.map((c, i) => <span key={i} className="px-2 py-1 bg-gray-50 text-xs rounded border border-gray-100">{c}</span>)}
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <p className="text-[10px] font-bold text-gray-400 uppercase">Recomendações</p>
                      {isProfessor && <button onClick={() => { setEditingComp(comp.competencia); setEditValue(comp.recomendacoes); }} className="text-[10px] text-emerald-600 font-bold hover:underline">Personalizar</button>}
                    </div>
                    {editingComp === comp.competencia ? (
                      <div className="space-y-2">
                        <textarea value={editValue} onChange={(e) => setEditValue(e.target.value)} className="w-full p-2 text-sm border-2 border-emerald-100 rounded-lg outline-none min-h-[60px]" />
                        <div className="flex justify-end gap-2"><button onClick={() => setEditingComp(null)} className="text-xs">Cancelar</button><button onClick={() => handleSaveEdit(comp.competencia, 'recommendation')} className="px-2 py-1 bg-emerald-600 text-white rounded text-xs font-bold">Salvar</button></div>
                      </div>
                    ) : <p className="text-sm text-gray-600 italic">"{comp.recomendacoes}"</p>}
                  </div>
                </div>
                ...
                {comp.questoes && comp.questoes.length > 0 && (
                  <div className="mt-8 pt-6 border-t border-gray-50">
                    <h5 className="text-[10px] font-bold text-gray-400 uppercase mb-4">Detalhamento das Questões</h5>
                    <div className="space-y-3">
                      {comp.questoes.map((q, qIdx) => (
                        <div key={qIdx} className={cn("p-4 rounded-xl border", q.acertou ? 'bg-emerald-50/50 border-emerald-100' : 'bg-red-50/50 border-red-100')}>
                          <div className="flex items-center justify-between mb-2">
                            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded", q.acertou ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700')}>Questão {q.id}</span>
                            <span className="text-[10px] font-bold text-gray-400">{q.acertou ? 'ACERTOU' : 'ERROU'}</span>
                          </div>
                          <p className="text-sm text-gray-800 font-medium mb-3">{q.enunciado || `Questão sobre ${comp.competencia}`}</p>
                          
                          {/* Feedback Tabs */}
                          <div className="flex items-center gap-2 mb-3">
                            <button onClick={() => setQuestionTab(`${comp.competencia}-${q.id}`, 'question')} className={cn("px-3 py-1 text-[10px] font-bold rounded", (activeQuestionTabs[`${comp.competencia}-${q.id}`] || 'question') === 'question' ? 'bg-white shadow' : 'text-gray-500')}>Pergunta</button>
                            <button onClick={() => setQuestionTab(`${comp.competencia}-${q.id}`, 'feedback')} className={cn("px-3 py-1 text-[10px] font-bold rounded", activeQuestionTabs[`${comp.competencia}-${q.id}`] === 'feedback' ? 'bg-white shadow' : 'text-gray-500')}>Feedback {q.professor_feedback && '✓'}</button>
                          </div>

                          {(activeQuestionTabs[`${comp.competencia}-${q.id}`] || 'question') === 'question' ? (
                            <div className="grid grid-cols-2 gap-4">
                              <div className="bg-white p-2 rounded border border-gray-100"><p className="text-[9px] text-gray-400 font-bold uppercase">Resposta: {q.resposta_aluno}</p></div>
                              <div className="bg-emerald-50 p-2 rounded border border-emerald-100"><p className="text-[9px] text-emerald-700 font-bold uppercase">Gabarito: {q.gabarito}</p></div>
                            </div>
                          ) : (
                                ...
                                <div className="space-y-3 bg-white p-3 rounded-lg shadow-sm border border-emerald-100">
                                  <div className="grid grid-cols-4 gap-4">
                                    <div className="col-span-1">
                                      <label className="text-[9px] font-bold text-gray-400 uppercase">Nota (0-10)</label>
                                      <input type="text" value={notaValue} onChange={(e) => setNotaValue(e.target.value)} className="w-full p-2 border rounded font-black text-center" />
                                    </div>
                                    <div className="col-span-3">
                                      <label className="text-[9px] font-bold text-gray-400 uppercase">Feedback</label>
                                      <textarea value={feedbackValue} onChange={(e) => setFeedbackValue(e.target.value)} className="w-full p-2 border rounded text-xs min-h-[60px]" />
                                    </div>
                                  </div>
                                  <div className="flex justify-end gap-2">
                                    <button onClick={() => setEditingQuestionFeedback(null)} className="text-xs">Cancelar</button>
                                    <button onClick={() => handleSaveEdit(comp.competencia, 'fullQuestionFeedback', feedbackValue, q.id, notaValue)} className="px-3 py-1 bg-emerald-600 text-white rounded text-xs font-bold">Salvar</button>
                                  </div>
                                </div>
                              ) : (
                                <div className="p-3 bg-white/50 rounded-lg">
                                  {q.professor_nota && <div className="inline-block px-2 py-1 bg-emerald-100 text-emerald-700 font-black rounded text-xs mr-3">{q.professor_nota}</div>}
                                  <p className="text-sm italic inline">"{q.professor_feedback || 'Nenhum feedback do professor.'}"</p>
                                  {isProfessor && <button onClick={() => { setEditingQuestionFeedback(`${comp.competencia}-${q.id}`); setFeedbackValue(q.professor_feedback || ''); setNotaValue(q.professor_nota || ''); }} className="ml-3 p-1 text-emerald-600 hover:bg-emerald-100 rounded transition-colors"><Pencil size={12} /></button>}
                                </div>
                              )}
                            </div>
                          )}
